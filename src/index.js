...
planaria.start({
  filter: {
    "from": 604000,
    "q": {
      "find": { "out.s2": "building on bsv!" },
      "project": { "out.s2": 1 }
    }
  },
  onmempool: async (e) => { await db.collection("a").insertMany([e.tx]) },
  onblock: async (e) => { await db.collection("a").insertMany(e.tx) },
  onstart: (e) => {
    return new Promise(async (resolve, reject) => {
      ...
      connect(() => {
        if (e.tape.self.start) db.collection("a").deleteMany({ "blk.i": { "$gt": e.tape.self.end } }).then(resolve)
        else resolve()
      })
    })
  }
